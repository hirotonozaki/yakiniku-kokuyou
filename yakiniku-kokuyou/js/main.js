...
(function(){
  ...
  /* Parallax — スクロール連動の縦オフセット
     - .parallax-photo の親要素の位置を基準に計算
     - prefers-reduced-motion 尊重
     - モバイル(<=767px)では無効化（iOSスクロール挙動の不安定さ＋負荷回避）
     - IntersectionObserver で画面内の要素だけを計算対象にする */
  const Parallax = {
    visible: new Set(),
    ticking: false,
    io: null,
    ...
    init() {
      ...
      const els = Array.from(document.querySelectorAll('.parallax-photo'));
      if (!els.length) return;

      // 画面内に入っている要素だけを Set に積む（無駄計算回避）
      this.io = new IntersectionObserver(entries => {
        entries.forEach(en => {
          if (en.isIntersecting) this.visible.add(en.target);
          else this.visible.delete(en.target);
        });
      }, { rootMargin: '20% 0px' });
    ...
    _run() {
      ...
      requestAnimationFrame(() => {
        ...
        this.visible.forEach(el => {
          ...
          // 中央が基準（0=画面中央 / -0.5=画面下端 / +0.5=画面上端）
          const ratio = (rect.top + rect.height / 2 - vh / 2) / vh;
          const offset = ratio * -30; // 上に動くほうが自然なので符号反転＋振幅を控えめに
